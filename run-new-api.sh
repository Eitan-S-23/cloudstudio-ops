...
if [[ -f "$APP_DIR/feishu-bot.py" && -f "$APP_DIR/feishu-bot-config.json" ]] \
  ...
  python3 -m pip install --quiet --disable-pip-version-check \
    ...
    || python3 -m pip install --quiet --disable-pip-version-check lark-oapi \
    || echo "WARN: lark-oapi 安装失败,跳过飞书机器人" >&2
  if python3 -c "import lark_oapi" >/dev/null 2>&1; then
    export APP_DIR LOG_DIR
    setsid bash -c 'while true; do python3 "$APP_DIR/feishu-bot.py" >>"$LOG_DIR/feishu-bot.log" 2>&1; echo "$(date "+%F %T") bot exited, restart in 10s" >>"$LOG_DIR/feishu-bot.log"; sleep 10; done' </dev/null >/dev/null 2>&1 &
  fi
fi

# new-api 放后台,task 前台保持一个交互 shell:
